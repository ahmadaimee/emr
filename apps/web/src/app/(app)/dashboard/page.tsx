  ...
  const data = await run('/dashboard', async (ctx) => {
    const [ar] = await ctx.tx.execute<{ total: string; over90: string; patient: string; open_claims: string; denied_30: string; adjudicated_30: string; clean: string; submitted_30: string; charges_90: string }>(sql`
      select coalesce(sum(balance_cents) filter (where status not in ('paid','closed','voided')), 0)::text as total,
        ...
        from claims
    `);
    const queues = await ctx.tx.execute<{ key: string; name: string; category: string; open: string; urgent: string; amount: string }>(sql`
    ...
    `);
    const activity = await ctx.tx.select().from(schema.activityEvents).orderBy(desc(schema.activityEvents.occurredAt)).limit(12);
    const [auto] = await ctx.tx.select().from(schema.automationSettings).where(sql`practice_id is null`);
    const [spend] = await ctx.tx.select({ cents: sql<number>`coalesce(sum(cost_cents),0)::bigint`, calls: sql<number>`count(*)::int` }).from(schema.externalCalls).where(sql`started_at >= date_trunc('day', now())`);
    const risk = await ctx.tx.select({ n: sql<number>`count(*)::int`, cents: sql<number>`coalesce(sum(balance_cents),0)::bigint` }).from(schema.claims).where(sql`timely_filing_deadline <= current_date + 14 and status not in ('paid','closed','voided','submitted','acknowledged','in_process')`);
          ...
          <Card title="Work queues">
            <table className="g-table">
              ...
              <tbody>
                {data.queues.map((q) => (
                  <tr key={q.key}>
                    <td><Link href={`/queues/${q.key}`} className="font-medium hover:underline">{q.name}</Link></td>
                ...
                <li key={a.id} className="px-4 py-2 text-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <span className="font-medium">{a.actorType === 'system' ? 'Grove' : a.actorLabel ?? 'Someone'}</span> <span className="text-ink-2">{a.summary}</span>
                    </div>
                    <span className="shrink-0 text-xs text-ink-4">{relative(a.occurredAt)}</span>
                  </div>
                  <Link href={`/${a.subjectType === 'eligibility_batch' ? 'eligibility' : a.subjectType === 'patient' ? 'patients' : a.subjectType === 'remittance' ? 'remittances' : 'claims'}/${a.subjectId}`} className="text-xs text-ink-3 hover:underline">
                    <StatusPill status={a.verb.split('.')[1] ?? a.verb} />
                  </Link>
                </li>
              ))}
            </ul>
