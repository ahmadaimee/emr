...
import { parseAddOnCodes, parseIcd10Order, parseMue, parseNcciPtp, parseReasonCodes } from './parsers';

const [set, file, effective] = process.argv.slice(2);
if (!set || !file) {
  console.error('usage: import <icd10cm|ncci_ptp|mue|addon|carc|rarc> <file> [effective-date]');
  process.exit(2);
}
...
try {
  await sql.begin(async (tx) => {
    ...
    await tx`insert into code_set_versions (code_set, version, effective_date, record_count, source_url, checksum) values (${set}, ${eff}, ${eff}, ${count}, ${file}, ${checksum}) on conflict (code_set, version) do update set record_count = excluded.record_count, checksum = excluded.checksum`;
  });
