...
`docs/` is an Obsidian vault expanding on everything above — why each rule exists, how
tenant isolation is enforced, and what the revenue-cycle terms mean. Start at
`docs/Grove Engineering.md`. Keep it current: a note that contradicts the code should be
corrected or deleted, not left as a second opinion.
